...
export default function ClientFavorites({ clientUser, addToCart, onLoginClick }) {
  ...
  const cardSx = {
    borderRadius: 5, overflow: 'hidden', bgcolor: '#FFFFFF',
    boxShadow: '0 4px 24px rgba(44,24,16,0.10)', transition: 'all 0.3s ease',
    '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 16px 40px rgba(44,24,16,0.18)' },
  ...
  return (
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 8 }}>
      ...
      {favorites.length === 0 ? (
        <Box sx={{ bgcolor: 'white', borderRadius: 5, boxShadow: '0 4px 24px rgba(44,24,16,0.10)', p: 8, textAlign: 'center' }}>
          <Favorite sx={{ fontSize: 56, color: '#FFCC80', mb: 2 }} />
          <Typography variant="h6" sx={{ color: "#2C1810" }} gutterBottom>
            Você ainda não tem produtos favoritos.
          </Typography>
          <Typography color="text.secondary" mb={3}>
            Explore o cardápio e salve seus cookies favoritos!
          </Typography>
          <Button variant="contained" component={Link} to="/" sx={{ borderRadius: 50, bgcolor: '#D4580A', '&:hover': { bgcolor: '#B84508' }, px: 4 }}>
            Ver Cardápio
          </Button>
        </Box>
      ) : (
            ...
            return (
              <Grid item xs={12} sm={6} md={4} key={prod.id}>
                <Box sx={{ ...cardSx, position: 'relative' }}>
                  ...
                  <Box sx={{ p: 2, pt: 0 }}>
                    <Button variant="contained" fullWidth startIcon={<AddShoppingCart />} onClick={() => handleAddWithPopup(prod)} sx={{ borderRadius: 50, bgcolor: '#D4580A', '&:hover': { bgcolor: '#B84508' } }} disabled={prod.estoque <= 0 || !isStoreOpen}>
      ...
      <Dialog open={crossSellOpen} onClose={() => setCrossSellOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 6, bgcolor: '#FFFAF5' } }}>
            ...
            {crossSellItems.map(prod => {
              ...
              return (
                <Grid item xs={6} key={prod.id}>
                  <Box sx={{ bgcolor: '#FFF8F0', borderRadius: 4, overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column' }}>
                    {coverImage && <Box component="img" src={coverImage} sx={{ width: '100%', height: 100, objectFit: 'cover' }} />}
                    ...
                    <Box sx={{ p: 1, display: 'flex', justifyContent: 'center' }}>
                      <Button size="small" variant="contained" onClick={() => addToCart(prod, 1)} sx={{ borderRadius: 50, bgcolor: '#D4580A' }}>
                        Adicionar
                      </Button>
                    </Box>
                  </Box>
        ...
        <DialogActions sx={{ justifyContent: 'center', pb: 3, gap: 2 }}>
          ...
          <Button onClick={() => { setCrossSellOpen(false); navigate("/carrinho"); }} variant="contained" sx={{ bgcolor: '#D4580A', color: 'white', borderRadius: 50 }}>
            Ver meu carrinho
          </Button>
        </DialogActions>
      </Dialog>
